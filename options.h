 ...
 */
#define DEFAULT_PRAGMAS PRAGMA_STRICT_TYPES + PRAGMA_ERROR_CONTEXT
 ...
 * 去掉每次命令结束时的 '> '
 */
#define NO_PRINTF_PROMPT
/**
 * 去掉解析主机地址的 addr_server_fd 的socket链接
 */
#define NO_IP_DEMON
/**
 * moon包添加以下函数
 * milli_clock - 取系统毫秒及时间、从启动开始连线
 * luan - 后台输出函数用于调试使用
 * MD5 - 单向加密函数，用于登录连线
 */
#define PACKAGE_MOON
...
/* epoll 监听时延、10毫秒没有socket事件、跳出wait */
#define EP_TIME_OUT			10
/* 每多少此事件、flush一次数据 */
#define EP_TICK_FLUSH		10
/*
 * 重新设置系统最大打开文件的数目
 * new_user_handle、将预留10个句柄给其他程序
 * 如、第一次load文件、编译文件、update文件、输出log
 * read文件、数据库句柄、lpc_socket、都需要系统分配句柄
 */
