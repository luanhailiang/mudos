...
static int      MySQL_affected PROT((dbconn_t *));

static db_defn_t mysql = {
    "MySQL", MySQL_connect, MySQL_close, MySQL_execute, MySQL_fetch, NULL, NULL, MySQL_cleanup, NULL, MySQL_errormsg, MySQL_affected
};
#endif
...
/* int db_affected(int handle)
 *
 *
 *
 * returns sqlstatement affected row number
 */
#ifdef F_DB_AFFECTED
void f_db_affected PROT((void))
{
	int ret = 0;
	db_t *db;
...
static char *MySQL_errormsg P1(dbconn_t *, c)
{
    ...
    return string_copy((char*)mysql_error(c->mysql.handle), "MySQL_errormsg:2");
}
...
static int MySQL_affected P1(dbconn_t *, c)
{
...
static array_t *MySQL_fetch P2(dbconn_t *, c, int, row)
{
	...
	if (!field) {
		...
		case FIELD_TYPE_LONGLONG:
		    ...
		    if(target_row[i])
		    	v->item[i].u.number = atol(target_row[i]);
		    else
		    	v->item[i].u.number = 0;
		    break;
...
static int MySQL_connect P5(dbconn_t *, c, char *, host, char *, database, char *, username, char *, password)
{
    ...
    *(c->mysql.errormsg) = 0;
	...
	c->mysql.handle = mysql_real_connect(tmp, host, username, password, database, 0, NULL, 0);
    if (!c->mysql.handle) {
...
static array_t *msql_fetch P2(dbconn_t *, c, int, row)
{
	...
	if (!field || !this_row[i]) {
		...
		case UINT_TYPE:
		    v->item[i].type = T_NUMBER;
		    v->item[i].u.number = atol(this_row[i]);
		    break;
