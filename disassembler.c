...
disassemble P5(FILE *, f, char *, code, int, start, int, end, program_t *, prog)
{
	...
	case F_LOOP_COND_NUMBER:
	    ...
	    offset = (pc - code) - (unsigned short) sarg;
	    pc += 2;
	    sprintf(buff, "LV%d < %ld bbranch_when_non_zero %04x (%04x)",
		    i, iarg, sarg, offset);
	    break;
	case F_LOOP_COND_LOCAL:
	    ...
	    offset = (pc - code) - (unsigned short) sarg;
	    pc += 2;
	    sprintf(buff, "LV%d < LV%ld bbranch_when_non_zero %04x (%04x)",
		    i, iarg, sarg, offset);
	    break;
	case F_STRING:
	    COPY_SHORT(&sarg, pc);
	...
	case F_SWITCH:
		...
		if (ttype == 0) {
			...
			if (ttype == 1 || !parg) {
			    if (sarg == 1)
				fprintf(f, "\t%-4lu\t<range start>\n", (unsigned long)parg);
			    else
				fprintf(f, "\t%-4lu\t%04x\n", (unsigned long)parg, addr+sarg);
			} else {
			    fprintf(f, "\t\"%s\"\t%04x\n",
...
#if defined(LPC_TO_C)
    COPY_LONG(&s, li);
#elif defined(USE_32BIT_ADDRESSES)
    COPY_INT(&s, li);
#else
    COPY_SHORT(&s, li);
#endif
	li += sizeof(ADDRESS_TYPE);
	fprintf(f, "%4x-%4x: %ld\n", addr, addr + sz - 1, (long)s);
	addr += sz;
    }
}
#endif
