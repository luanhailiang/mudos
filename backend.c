...
#ifdef EPOLL
struct epoll_event event[EP_MAX_EVENT];
#endif
...
#ifdef EPOLL
	if (heart_beat_flag)
		nb = epoll_wait(epoll_fd, event, EP_MAX_EVENT , 0);
	else
		nb = epoll_wait(epoll_fd, event, EP_MAX_EVENT , EP_TIME_OUT);
	for(i = 0; i < nb ; i++)
	{
		process_io(&event[i]);
	}
