...
#ifdef INCL_SCHED_H
static void bind_cpu(int cpuidx)
{
	...
	{
		debug_message("warning: could not set CPU affinity\n");
	}

	CPU_ZERO(&mask);
	if (sched_getaffinity(0, sizeof(mask), &mask) == -1)
	{
		debug_message("warning: I could not get cpu affinity, continuing...\n");
	}
	else
	{
		if (CPU_ISSET(cpuidx, &mask) )
		{
			debug_message("\nthis process %d is running on processor: %d\n",getpid(),cpuidx);
		}
	}
}
#endif
...
#ifdef DEBUGMALLOC
char *int_string_copy P2(const char *, str, char *, desc)
#else
char *int_string_copy P1(const char *, str)
#endif
{
    char *p;
    int len;
