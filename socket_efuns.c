...
int socket_listen P2(int, fd, svalue_t *, callback)
{
    ...
    current_object->flags |= O_EFUN_SOCKET;
#ifdef EPOLL
    epoll_ctl_add(lpc_socks[fd].fd , EPOLLIN , (uint32_t)(PACKAGE_SOCKETS_FD | fd));
#endif
...
int socket_accept P3(int, fd, svalue_t *, read_callback, svalue_t *, write_callback)
{
	...
	current_object->flags |= O_EFUN_SOCKET;
#ifdef EPOLL
	epoll_ctl_add(lpc_socks[i].fd , EPOLLIN , (uint32_t)(PACKAGE_SOCKETS_FD | i));
#endif
