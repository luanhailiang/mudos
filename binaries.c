...
#endif
{
    ...
    object_t *ob;
    struct stat st;
    size_t rlen;	/* for warn_unused_result add by luan */
     ...
     */
	rlen = fread((char *) &len, sizeof len, 1, f);
    ALLOC_BUF(len);
	rlen = fread(buf, sizeof(char), len, f);
     ...
     */
	rlen = fread((char *) &ilen, sizeof ilen, 1, f);
    p = (program_t *) DXALLOC(ilen, TAG_PROGRAM, "load_binary");
	rlen = fread((char *) p, ilen, 1, f);
	...
	rlen = fread((char *) p->file_info, len, 1, f);
    p->line_info = (unsigned char *)&p->file_info[p->file_info[1]];

    /* patches */
	rlen = fread((char *) &len, sizeof len, 1, f);
    ALLOC_BUF(len);
	rlen = fread(buf, len, 1, f);
    /* fix up some stuff */
